...
export function useMonitoringData() {
  ...
  onMounted(async () => {
    ...
    try {
      ...
      unsubscribe = fb.onValue(reference, (snapshot: any) => {
        if (snapshot.exists()) {
  ...
  const triggerFeeding = async (): Promise<void> => {
    ...
    try {
      ...
      await Promise.all([
        ...
        fb.set(fb.ref(database, 'ponds/pond_01/controls/manual_feeding/trigger'), true)
      ])
    } catch (e) {
      error.value = e as Error
      throw e
...
export function useFeedingSchedule() {
  ...
  const updateSchedule = async (scheduleKey: 'schedule_1' | 'schedule_2', time: string): Promise<void> => {
    if (import.meta.server) return
    ...
    try {
      ...
      await fb.set(fb.ref(database, `schedules/feeding_time/${scheduleKey}`), time)
    } catch (e) {
      error.value = e as Error
      throw e
    }
